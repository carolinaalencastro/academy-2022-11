with
    ...
    , transformacoes as (
        select 
            row_number() over (order by id_produto) as sk_produto --criar uma chave única para cada linha, irá fazer uma contagem de linhas por ordem crescente
            ,*
        from uniao_tabelas
    )
